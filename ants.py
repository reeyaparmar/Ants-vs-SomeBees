...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit == None: #if the place has an exit, then the exit's entrance is set to that Place
            self.entrance = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        current = self.place
        distance = 0
        while current is not None and current.is_hive == False:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown > 0: #decrement cooldown if its chewing
            self.cooldown = self.cooldown - 1
        else:
            if self.place.bees:
                eaten = random_bee(self.place.bees)
                eaten.reduce_health(eaten.health)
                self.cooldown = self.chew_cooldown #after ant has eaten, set cooldown score back to 3
# END Problem 7
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current is not None:
            if current.ant is not None: #double the damage of ants behind queen
                current.ant.double()
